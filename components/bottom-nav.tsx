...
export function BottomNav() {
  ...
  return (
    ...
    >
      <div className="relative flex w-full max-w-sm items-center justify-between rounded-full border border-border bg-card px-3 py-2 shadow-lg shadow-primary-foreground/10">
        {leftItems.map((item) => (
        ...
        <Link
          ...
          className="absolute left-1/2 top-0 flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 touch-manipulation items-center justify-center rounded-full bg-primary-foreground text-white shadow-md shadow-primary-foreground/40 transition-transform active:scale-95"
        >
          <PlusIcon />
        </Link>
      </div>
...
function NavLink({ item, active }: { item: NavItem; active: boolean }) {
  return (
    <Link
      href={item.href}
      className={`flex w-12 touch-manipulation flex-col items-center gap-0.5 rounded-2xl py-1.5 text-[10px] transition-colors ${
...
function OverviewIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5" aria-hidden="true">
      ...
      <path d="M12 3v9l7.79 4.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
